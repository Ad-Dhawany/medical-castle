...
class order{
    ...
    function __construct(&$isSuccess = 0, $orderID=null, $costumerID=null,
                        $procStatus=null,  $discountRatio=null, $createdDate=null/* , $createdBy =null */)
            {
                ...
                || (isset($costumerID ) && (!isset($this->costumerID ) || $this->costumerID  === false)) || (isset($discountRatio) && !isset($this->discountRatio))){
                    $isSuccess = 0;
                }else $isSuccess = true;
    ...
    /*******************************************/
    /** */
    /* function getOrderByID($incVersion = true){
        if($incVersion){
            return $this->getObjectPropsByID(self::VIEW_NAME, 'orderID', 'orderVersion');
        }else{
            return $this->getObjectPropsByID(self::VIEW_NAME, 'orderID');
        }
    } */
