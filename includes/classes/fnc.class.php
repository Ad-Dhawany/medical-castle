...
/* GetTitle function to insert Title of current page into header */
    ...
    static function redirectHome($errorMsg, $url = null, $second = 4, $class = 'danger'){
        global $dir;
        if($url == null){
            $url = BASIC_ADDRES/* (isset($dir))? $dir. "home/" : "home/" */;
            $link = 'Home Page';
        }elseif($url == 'back'){
        ...
        echo "<div class='fa-pull-right my-3'><a href='". $url. "' class='btn btn-secondary' autofocus>Back</a> <a href='". BASIC_ADDRES /* ((isset($dir))? $dir. "home/" : "home/") */. "' class='btn btn-secondary'>Home Page</a></div>";
        echo "</div>";
    ...
    */
    ...
    static function isArabicStr($string, $incSpace = true ,$incNum = true ,$justAr = true){
        ...
        /* echo "<div class='m-5'> strlen : ". strlen($string). ' <br> $c : '. $c. "</div>"; */ /** Just for testing */
        if($c < 2){
           return false;
    ...
    /*******************************/
    /** */
    static function isTownExistInConstArray($town){
        $cond = false;
        foreach(MUNICIPALS_TOWNS as $muniTown){
            if(is_array($muniTown)){
                if(in_array($town, $muniTown)){
                    $cond = true;
                    return $cond;
                }
            }
