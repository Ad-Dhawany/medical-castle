...
class orderItem{
    ...
    function getOrderItemByID(){
        return $this->getObjectPropsByID(self::VIEW_NAME, 'ID');
    }
    /*******************************************/
    /** */
    ...
    function getTotalItemPrice($discRatio){
        if(is_numeric($discRatio) && $this->orderVersion == 1){
            return number_format((doubleval($this->totalItemPrice) * (1 - doubleval($discRatio) / 100)), 2);
        }else{
            return number_format((doubleval($this->totalItemPrice)), 2);
        }
    }
    /*******************************/
}