...
ob_start();
    ...
    if($do == 'manage'){
        ...
        $costumers = stat_getObjects::getMembers($where, 'costumer');
        ?>
        ...
        $ID = (isset($_GET['ID']) && is_numeric($_GET['ID'])) ? intval($_GET['ID']) : fnc::redirectHome("خطأ: 6101",'back',0);
        ...
        if ($count){
            $lessGroupIdToChangePass = setting::getSpecificSetting("leastGroupIdCanChangeCostumerPass")["value"];
            $canChangePass = ($_SESSION['groupID'] >= $lessGroupIdToChangePass) ? true : false ; ?>
            <h1 class="text-center m-3"><?php echo lang('EDIT'); ?> <span style="color:darkslategray" title="<?php echo lang("USERNAME"). ": ". ucwords($costumer->username) ?>"><?php echo ucwords($costumer->fullname) ?></span> <?php echo lang('INFORMATION'); ?></h1>
            <div class="container">
                ...
                <form action="./?do=update" method="POST" class="form row g-1">
                    <div style="display: none;"><input type="hidden" name="id" class="form-control" value="<?php echo ucwords($costumer->userID)?>"></div>
                    <div class="form-group form-group-lg row mb-2">
                        <label class="col-sm-2 control-label" title="<?php echo lang("USERNAME"). ": ". ucwords($costumer->username) ?>"><?php echo lang("FULLNAME") ?></label>
                    ...
                    <div class="form-group form-group-lg row mb-2">
                        ...
                        <div class="input-field col-sm-10 col-md-6">
                            <input type="password" name="password" class="form-control"  autocomplete="new-password" value="********" disabled>
                        </div>
                    </div>
        ...
        if(isset($_POST['id'],$_POST['fullname']/* , $_POST['password'] */, $_POST['email'], $_POST['phone'], $_POST['pharmacy'])){
            ...
            if(empty($formErrors)){
            $costumerUb = new costumer($e, $_POST['id'], null/* strtolower($_POST['username']) */, $_POST['email'],$_POST['phone'], strtolower($_POST['fullname']),null,null,null,null,null,strtolower($_POST['pharmacy']));
            $realGroupID = $costumerUb->getMemPropByProp("groupID", 'userID', 'userID');
            if(!$e){
                fnc::redirectHome("ex0217", 'back', 0);
            }
            if($realGroupID !== costGroupID){
                fnc::redirectHome("ex0220", 'back', 3);
            }
            /* $costumerUb->preparePhone(); */
            if(fnc::checkitem("userID",'users',$costumerUb->userID) != 1){
                fnc::redirectHome("x0215", 'back', 0);
            }
            if($costumerUb->isThereUniqueProbInAnotherMemb($prob, 'userID', 'phone', 'email', 'pharmacy') != 0){
            ...
            if(!empty($_POST['new-password'])){
                ...
                if($canChangePass){
                    ...
                    fnc::redirectHome("خطأ: ex0242", 'back', 0);
                }
            }/* else{
            ...
            /* $count  = $costumerUb->membLoign(null,null,$e,"userID","verify");
            if ($count){ */ // confirming success
                ...
                if($count2){
                    $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, $notiTybe, 2, $costumerUb->userID);
                    $notification->insertNotification($e, 'ALL');
                    $successMsg = '<h4 class="text-center">The updating recorded successfully </h4>';
                    fnc::redirectHome($successMsg, 'back', 1, 'success');
                }else{
                    fnc::redirectHome($e, 'back',2);
                }
            /* }else{
    ...
    <?php }elseif($do == 'insert'){
        ...
        if(isset($_POST['username'],$_POST['fullname'], $_POST['password'], $_POST['phone'], $_POST['email'], $_POST['pharmacy'])){
            ...
            //insert new user
                ...
                if ($count){
                    $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, "costumer//user//insert", 2, $costumerNew->userID);
                    $notification->insertNotification($e, 'ALL');
            ...
            if ($count){
                if($costumerDel->getGroupID() === costGroupID){ /* if this costumer has admin permissions privent*/
                    if($costumerDel->deleteMemb($e, "userID")){
                        ...
                        $delMembLog->writedeleteMemberLog($costumerDel->userID, $costumerDel->username, $costumerDel->getGroupID());
                        $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, "costumer//user//delete", 3, $costumerDel->userID);
                        $notification->insertNotification($e, 'ALL');
        ...
        echo "<div class='container'>";
            ...
            $count  = $costumerAct->getMemberByID(); // if can't get a member by th receiving ID will return false; by this way we can aprov if member exist or not
            if ($count){
                ...
                if($costumerAct->updateMemb($e, "userID", "regStatus", "lastActiveBy", "lastActiveDate")){
                    $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, "costumer//user//activate", 2, $costumerAct->userID);
                    $notification->insertNotification($e, 'ALL');
    ...
    echo "</div>";
        ...
        $costumerTr = new costumer($e, $_GET['ID']);
            if(!$e){
                fnc::redirectHome('e.0x6490',"back",0);
            }
            ...
            if ($cound){
                ...
                if($costumerTr->getGroupID() === costGroupID){ /* if thid member has admin permissions privent*/
                    if($costumerTr->updateMemb($e, "userID", "regStatus", "regStatus", "lastTrushBy","lastTrushDate")){
                        $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, "costumer//user//trash", 2, $costumerTr->userID);
                        $notification->insertNotification($e, 'ALL');
            ...
            if ($cound){
                ...
                if($costumerTr->getGroupID() === costGroupID){ /* if thid member has admin permissions privent*/
                    if($costumerTr->updateMemb($e, "userID", "regStatus", "regStatus", "lastPendBy","lastPendDate")){
                        $notification = new broadNotifications($is, null, $_SESSION['userID'], -1, "costumer//user//block", 2, $costumerTr->userID);
                        $notification->insertNotification($e, 'ALL');
        ...
        <?php
            ...
            if($count){
        ?>
        <main class='' data-page-title="<?php echo lang('COS INF')?>">
            <h1 class="text-center"><?php echo lang('COS INF'). " <span class='text-secondary'>(" .ucwords($costumer->username). ")</span> " ?></h1>
            <div class="container">
                <div class="row">
