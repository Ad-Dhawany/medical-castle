...
trait queryTraits{/* */
    ...
    /********************************************/
    /** */
    function getObjectPropsByID($table , $IDpropName, $anotherProp = null,$joinTable='', $joinBy=[], ...$joinGet){
        global $conn;
        if(isset($this->$IDpropName) && is_numeric($this->$IDpropName)){
            ...
            if(is_array($joinBy) && count($joinBy) == 2){
                $join = " INNER JOIN `$joinTable` t2 ON t1.". $joinBy[0]. " = t2.". $joinBy[1] ;
                $joinSelect = ",t2.". implode(',t2.', $joinGet);
            }
            /* echo "SELECT t1.* $joinSelect FROM `$table` t1 $join WHERE $where"; */
            /*fnc::prePrint($bind); */
            $stmt = $conn->prepare("SELECT t1.* $joinSelect FROM `$table` t1 $join WHERE $where");
            $stmt->execute($bind);
            $result = $stmt->fetch(2);
    ...
    /**********************************/
    /** */
    function getObjectPropWhere($table, $get, $comparision = array(), $where=array(), $seperator = "AND"){
        global $conn;
    ...
    /**********************************/
    /** */
    function getObjectPropByJoin($fromTable, $joinTable, $joinBy=[], $joinGet){
        if(is_array($joinBy) && count($joinBy) == 2){
            $propName = $joinBy[0];
            ...
            global $conn;
            $join = " INNER JOIN `$joinTable` t2 ON t1.". $joinBy[0]. " = t2.". $joinBy[1] ;
            $stmt = $conn->prepare("SELECT `t2`.`$joinGet` FROM `$fromTable` t1 $join WHERE $joinBy[0] = ?");
            $stmt->execute(array($this->$propName));
            $result = $stmt->fetch();
            $count  = $stmt->rowCount();
