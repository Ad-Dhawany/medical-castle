$(function(){
    ...
    */
    ...
    var allItems = [], defaultDiscRatio = 3,
        orderItmes = [], maxDiscRatio = 6, injectMode = 'insert', 
   ...
   var trClicks = 0;
    ...
    /*******************************************/
    /** */
    $("#submit-reply-btn").click(async function(){
        if(orderItmes.length > 0){
            if(await $(this).myConfirm()){
                if(saveOrder('reply')){
                    setTimeout(function(){
    ...
    /**********************/
    /** */
    $("#discount-ratio").on('input',function(){
        if($(this).val() > maxDiscRatio){
            $(this).val(maxDiscRatio);
        }
        var netTotal = totalPrice * (1- $(this).val()/100) ;
        $("#order-net-total").text(netTotal.toFixed(2));
    });
   ...
   function itemSearch(text, method = 'name'){
    ...
    $.each(matchedItems , function (index, matchedItem) {
        ...
        $(".result-items").append(`<tr tabindex="0" data-orderItem-id="`+ matchedItem.itemID+ `" class="pointer result-item-row">
                                ...
                                <td title = "`+ discPriceTitle+`">`+ (matchedItem.salePrice - (matchedItem.salePrice * (defaultDiscRatio / 100))).toFixed(2) + ` د.ل </td></tr>`);
    });
   }
   /**************************************/
    ...
    function fillOrderTable(){
        ...
        for(let i=0; i<orderItmes.length; i++ ){
            ...
            tbodyItemsSelector.append(`<tr class='select-item' data-item-index='${i}'>
                                                ...
                                                <td>${orderItmes[i].insertDate.slice(0,-3)}</td>
                                            </tr>`);
            totalPrice += totalItemPrice;
        }
        $("#order-sub-total").text(totalPrice.toFixed(2));
        var netTotal = totalPrice * (1- $("#discount-ratio").val()/100) ;
        $("#order-net-total").text(netTotal.toFixed(2));
    }
    /******************************************/
    /** */
    ...
    function ajaxItems(){
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                defaultDiscRatio = response;
            }
        });
        /** get maximum discount ratio */
        $.ajax({
            type: "POST",
            url: "../api/getSetting.api.php",
            data: {'target': 'maximumDiscRatio'},
            dataType: "json",
            success: function (response) {
                maxDiscRatio = parseFloat(response);
            }
        });
    }
    /*************************/
